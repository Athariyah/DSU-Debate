...
На маленьких VPS (1 ГБ RAM и меньше) скрипт заодно создаёт файл подкачки
`/swapfile` (1 ГБ, переопределяется `SWAP_SIZE_MB`): без него сборка
фронтенда и пики нагрузки упираются в OOM-Killer. Если swap уже включён или
виртуализация его запрещает (OpenVZ/LXC) — шаг пропускается с предупреждением.
...
**«ts-node в продакшене» (частая ошибка ручного запуска).** Если в `htop`
виден процесс вида `ts-node src/server.ts`, запущенный под `root`, — backend
работает в режиме разработки, в обход всего, что репозиторий готовит для
продакшена. Чем это плохо:

* `ts-node` компилирует TypeScript на лету: тот же код держит в памяти в
  3–4 раза больше (~220 МБ `RES` против десятков у собранного `dist`) и
  медленнее отвечает на запросы;
* процесс живёт под `root`: любая уязвимость в коде отдает атакующему весь
  сервер, а не каталог приложения;
* при падении никто не поднимет процесс (systemd-юнит с `Restart=always`
  делает это через 3 секунды);
* запуск мимо systemd — это и мимо `/etc/dsu-debate/backend.env`: секреты и
  настройки не подгружены, поведение зависит от случайных переменных окружения.

Как правильно (всё это делает `deploy/setup.sh` за один запуск):

```bash
cd /opt/dsu-debate/backend && npm run build   # TypeScript -> backend/dist/*.js
sudo systemctl restart dsu-debate-backend     # node dist/server.js от пользователя dsu
pkill -f 'ts-node src/server.ts'              # убить забытый ручной процесс, если он есть
```

Проверка: `systemctl status dsu-debate-backend` показывает `node .../dist/server.js`,
владелец процесса — пользователь `dsu`, а не `root`.
