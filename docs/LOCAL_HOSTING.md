...
PostgreSQL. Источники, откуда открыт сайт, backend принимает без настройки: в
режиме разработки по умолчанию разрешены любые (Live Server на 127.0.0.1,
localhost, IP в локальной сети, туннели, превью-домены). Политика видна в логе
backend строкой `CORS: ...`; ограничить список можно переменной `CORS_ORIGIN`.
...
3. API по умолчанию принимает любые источники, дополнительно настраивать CORS
   не нужно. Если вы ограничивали доступ списком (`CORS_ORIGIN=http://...`),
   добавьте туда адрес сайта либо задайте `CORS_ORIGIN=*`.
...
| Ошибка CORS в консоли браузера | По умолчанию в режиме разработки разрешены **любые** источники, так что этот вариант возможен только если вы сами задали `CORS_ORIGIN` списком. Тогда добавьте в него источник со страницы (видно в тексте ошибки) или верните `CORS_ORIGIN=*`. Какой источник отклонён и что делать — backend пишет в свой лог строкой `[cors] отклонён запрос...`. |
| `CORS_ORIGIN_NOT_ALLOWED` (403), а раньше была «внутренняя ошибка» | Так теперь выглядит отказ по источнику: понятный код и подсказка вместо 500. Действия те же — добавить источник в `CORS_ORIGIN` либо поставить `*`. |
...
| `CORS_ORIGIN` | любые источники в dev, явный список в production | откуда браузеру разрешено обращаться к API |
