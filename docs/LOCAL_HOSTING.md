...
Терминал должен остаться открытым — это и есть сервер приложения. Закрытие
терминала (Ctrl+C) корректно останавливает и API, и PostgreSQL; **данные
остаются** в `backend/.localdb`.
...
Доступ администратора (создаётся один раз при первом старте):
...
Смените пароль перед публикацией в интернет: `ADMIN_PASSWORD` в `backend/.env`
(файл-образец — `backend/.env.example`).
