...
if su -s /bin/sh "$PG_USER" -c "$PG_BINDIR/pg_isready -h 127.0.0.1 -p 5432 -q" 2>/dev/null; then
  ...
  (
    while :; do
      su -s /bin/sh "$PG_USER" -c "$PG_BINDIR/postgres -D $PGDATA -c listen_addresses=127.0.0.1"
      echo "[embedded] postgres exited with code $?, restarting in 2s" >&2
      sleep 2
    done
  ) &
  PG_LOOP_PID=$!
...
# --- Backend: run under a respawn loop (migrations + admin seed at start) ---
trap 'if [ -n "${PG_LOOP_PID:-}" ]; then kill "$PG_LOOP_PID" 2>/dev/null; fi; exit 0' TERM INT
echo "[embedded] starting backend on port ${PORT:-4000}" >&2
while :; do
  node /app/dist/server.js
  echo "[embedded] backend exited with code $?, restarting in 2s" >&2
  sleep 2
done
