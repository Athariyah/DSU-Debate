...
### Устойчивость realtime: polling-подстраховка

- `src/pages/DebateDetailPage.tsx` — если realtime-канал не жив (WebSocket зарезан прокси/DPI, мобильная сеть нестабильна), страница зрителя перечитывает результаты по REST раз в 5 с (`FALLBACK_POLL_MS`), как это уже делала страница трансляции (`BroadcastPage`). Цифры больше не «замерзают» до перезагрузки; индикатор соединения честно пишет «Обновление каждые 5 с». Фоновая перечитка идёт без скелетона и при разовом сбое не затирает уже показанное мероприятие.

### Совместимость с ограниченными шлюзами: POST-алиасы

- `backend/src/routes/admin.*.routes.ts` — для всех админских PUT/DELETE добавлены POST-алиасы тех же обработчиков (`POST .../:id/update`, `POST .../:id/delete`): часть прокси и CDN блокирует PUT/DELETE и отвечает 405. PUT/DELETE сохранены для обратной совместимости.
- `src/api/debates.ts` — клиент (обновление/удаление событий, участников, матчей) шлёт POST-алиасы.

### Эксплуатация

- `deploy/setup.sh` — автоматическое создание swap-файла (`/swapfile`, 1 ГБ, размер — `SWAP_SIZE_MB`) на серверах без подкачки: защита от OOM-Killer при сборке фронтенда и пиках нагрузки. Шаг пропускается, если swap уже включён или виртуализация его запрещает.
- `docs/VPS_DEPLOY.md` — раздел «ts-node в продакшене»: почему ручной запуск `ts-node src/server.ts` под `root` (память x3–4, нет авторестарта, секреты мимо systemd) — ошибка, и как правильно (`npm run build` + `systemctl restart dsu-debate-backend`).
