...
### Пустые состояния: текст на фоне вместо «панелек»

- `src/pages/HomePage.tsx`, `src/pages/DebatesPage.tsx` — сообщения пустых списков («Сейчас нет активных мероприятий. Загляните позже!», «Пока нет запланированных мероприятий», «Завершённых мероприятий пока нет», «История пока пуста») больше не карточки: убраны `glass-panel`, рамка и скругление, текст остался на фоне страницы и отцентрован (`text-center`); в сетке списка — на всю её ширину (`col-span-full`).
- `src/pages/AdminPage.tsx` — сообщение «Мероприятий пока нет» / «Нет мероприятий этого типа» оформлено так же: голый текст на фоне, по центру.

### Центрирование контента на широком экране

- `src/pages/SplashPage.tsx`, `src/pages/DebateDetailPage.tsx`, `src/pages/AdminPage.tsx`, `src/pages/CreateDebatePage.tsx` — убран отступ `lg:pl-64`, зарезервированный под боковую навигацию. Эти экраны не рисуют боковое меню (оно есть только на «Главной», «Мероприятиях» и «Профиле»), а отступ оставался — колонка контента и заголовок шапки смещались вправо от центра окна. Теперь контент этих страниц стоит посередине экрана.

### Тесты

- `src/pages/EmptyStates.test.tsx` — пустые состояния проверяются как текст на фоне (без `glass-panel`/рамки/скругления) с выравниванием по центру.
