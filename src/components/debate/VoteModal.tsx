...
interface VoteModalProps {
  ...
  open: boolean;
  onClose: () => void;
  onVoted: (participantId: string) => void;
...
export function VoteModal({ event, open, onClose, onVoted, preselectedParticipantId }: VoteModalProps) {
  ...
  const [participantId, setParticipantId] = useState<string>(preselectedParticipantId ?? "");
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setVotedParticipant(event.id, participantId);
      onVoted(participantId);
    } catch (error) {
      const anyErr = error as { status?: number; message?: string };
      if (anyErr?.status === 409) {
        // Anti-fraud: уникальный индекс по fingerprint/ip уже сработал на бэкенде
        setVotedParticipant(event.id, participantId);
        onVoted(participantId);
      } else {
        setApiError(anyErr?.message ?? "Не удалось отправить голос. Попробуйте ещё раз.");
      }
    } finally {
      setSubmitting(false);
    }
                ...
                {event.participants.map((p) => (
                  ...
                  >
                    <div>
                      ...
                      {p.subtitle && <p className="text-xs text-white/45">{p.subtitle}</p>}
                    </div>
                    <div
                      className={cn(
                        "h-4 w-4 shrink-0 rounded-full border-2",
                        participantId === p.id ? "border-indigo-400 bg-indigo-400" : "border-white/25"
                      )}
                    />
                  </button>
                ))}
