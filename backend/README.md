...
аккаунт никогда не перезаписывается). Вход выполняется во фронтенде через раздел
«Профиль»; JWT также ставится в HttpOnly cookie, Bearer-токен оставлен для
совместимости с ручными API-клиентами.
