...
-- У прикладной роли может не быть прав суперпользователя, а само расширение —
-- отсутствовать в сборке PostgreSQL (актуально для встроенной локальной БД и
-- для части managed-кластеров) — тогда установку пропускаем с предупреждением:
-- приложение использует UUID, сгенерированные на стороне Node, и от pgcrypto
-- не зависит.
DO $$
BEGIN
  BEGIN
